...
function PlayerGamingPage(){
		...
		if(cookies.roomName !== undefined){
			...
			.then(data => {
				...
				if(data[0].status ==="play"){
					fetch(`${localIP}/question-selected-room?roomID=${cookies.roomID}`)
					.then(response => response.json())
					.then(question => {
					...
					const url =`${localIP}/category-selected-room?roomID=${cookies.roomID}`
					fetch(url)
					.then(response => response.json())
					.then(data => {
	...
	const wrapperDonePlaying = useCallback(val =>{
		const data = {roomName: cookies.roomName, playerName: cookies.playerName}
		SocketIO.emit('update-status', data)
	})
	...
	useEffect(()=>{
		function updateStatus(){
			console.log("useEff update status")
			getStatus()
		}
	...
	const done = ()=>{
		...
		history.push(`/player-login?roomName=${cookies.roomName}&room_id=${cookies.roomID}`)
	}
	...
	const Display=()=>{
		...
		case "end":
			return <Scores roomName={roomName}/>
		default:
...
export default PlayerGamingPage;